...
import { uploadExcel, type UploadExcelResponse, getAllUploadStatus, startScrapBol, getAllScrapBolStatus, type ScrapBolStatusItem } from '@/app/api/3plGigaFedexApi';
import { ErrorDisplay } from '@/app/utils/Errors/ErrorDisplay';

import type { Order } from '@/app/types/order';
import type { ExcelPreviewData } from '../utils/excelFileGenerator';
import { blobToFile } from '../utils/excelFileGenerator';
...
type ParcelModalProps = {
  ...
  orders: Order[];
  onClose: () => void;
  subSKUsMap?: Record<number, string[]>; // Optional: SubSKUs from AutomateLogisticModal state
  initialExcelBlob?: Blob | null; // Optional: Pre-generated Excel file
  initialExcelPreviewData?: ExcelPreviewData | null; // Optional: Preview data
};

const CACHE_KEY = 'parcelModal_bolFormData';

export const ParcelModal = ({ isOpen, onClose, orders, subSKUsMap, initialExcelBlob, initialExcelPreviewData }: ParcelModalProps) => {
  // Note: orders and subSKUsMap are kept in props for interface consistency but are not currently used in this component
  ...
  const [excelPreviewData, setExcelPreviewData] = useState<ExcelPreviewData | null>(initialExcelPreviewData || null);
  const [excelBlob, setExcelBlob] = useState<Blob | null>(initialExcelBlob || null);

  // BOL Scraping Form Fields
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');
  const [dateFilterType, setDateFilterType] = useState<'both' | 'successfulBuyLabel' | 'creationDate'>('successfulBuyLabel');
  ...
  // Load cached BOL form data on mount and set initial Excel file
  useEffect(() => {
    if (isOpen) {
      try {
        ...
        if (cached) {
          ...
          setDateFilterType(parsed.dateFilterType || 'successfulBuyLabel');
      ...
      // Set the Excel file from initial blob if available
      if (initialExcelBlob) {
        const generatedFile = blobToFile(
          initialExcelBlob,
          `parcel_orders_${new Date().toISOString().split('T')[0]}.xlsx`
        );
  ...
  }, [isOpen, initialExcelBlob, initialExcelPreviewData]);
  ...
  // Get max date (2 days from today)
  ...
  const pollBolScrapingStatus = useCallback(async (scrapingId: string) => {
    try {
  ...
  }, [excelUploadProgress, clearCache]);

  // Start BOL scraping automatically after Excel upload
  const startBolScraping = useCallback(async () => {
    try {
  ...
  }, [startDate, endDate, dateFilterType, handle, pollBolScrapingStatus]);

  // Poll for Excel upload status
  const pollExcelUploadStatus = useCallback(async (uploadId: string) => {
    try {
  ...
  }, [startDate, endDate, scrapingStarted, startBolScraping]);
  ...
  }, [response?.upload_id, pollExcelUploadStatus]);
  ...
  }, [scrapingStarted, bolScrapingStatus?.scraping_id, pollBolScrapingStatus]);
  ...
  return (
    <div
      className="fixed inset-y-0 left-56 sm:left-64 right-0 z-40 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in"
      onClick={handleClose}
    >
      <div
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* Excel File Info - Show number of items */}
            {excelBlob && excelPreviewData && (
              <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <div className="flex items-center gap-2">
                  <FileSpreadsheet size={20} className="text-blue-600" />
                  <div>
                    <p className="text-sm font-semibold text-blue-800">
                      Excel file ready with {excelPreviewData.totalRows} item{excelPreviewData.totalRows !== 1 ? 's' : ''}
                    </p>
                    <p className="text-xs text-blue-600 mt-1">
                      File size: {(excelBlob.size / 1024 / 1024).toFixed(2)} MB
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  <div
                    className={`h-full rounded-full transition-all duration-500 ${
                      excelUploadProgress === 100
                        ...
                        : 'bg-linear-to-r from-blue-500 to-cyan-600 animate-pulse'
                    }`}
                  ...
                  <div
                    className={`h-full rounded-full transition-all duration-500 ${
                      bolScrapingProgress === 100
                        ? 'bg-linear-to-r from-green-500 to-emerald-600'
                        : 'bg-linear-to-r from-purple-500 to-pink-600 animate-pulse'
                    }`}
            ...
            {(scrapingStarted || scrapedCount > 0 || savedCount > 0) && (
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-linear-to-br from-green-50 to-emerald-50 rounded-xl border-2 border-green-200 p-4">
                ...
                <div className="bg-linear-to-br from-blue-50 to-cyan-50 rounded-xl border-2 border-blue-200 p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
      ...
      {/* Excel Preview Modal - Shows the same Excel file that will be generated */}
      <ExcelPreviewModal
        ...
        onUpload={async (file) => {
          // After preview, set the file in the input for upload
          setFile(file);
          ...
          // Close preview modal after setting file
          setExcelPreviewOpen(false);
          setExcelPreviewData(null);
          setExcelBlob(null);
        }}
