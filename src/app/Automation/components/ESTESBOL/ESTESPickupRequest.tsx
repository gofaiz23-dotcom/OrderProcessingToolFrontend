...
export const ESTESPickupRequest = ({ bolData, onSuccess, onCancel }: ESTESPickupRequestProps) => {
  ...
  useEffect(() => {
    if (bolData) {
      ...
      if (bolData.hazmat !== undefined) setHazmat(bolData.hazmat);
      if (bolData.protectFromFreezing !== undefined) setProtectFromFreezing(bolData.protectFromFreezing);
      if (bolData.food !== undefined) setFood(bolData.food);
      if (bolData.poison !== undefined) setPoison(bolData.poison);
      if (bolData.overlength !== undefined) setOverlength(bolData.overlength);
      if (bolData.liftgate !== undefined) setLiftgate(bolData.liftgate);
      if (bolData.doNotStack !== undefined) setStackable(!bolData.doNotStack); // Inverted: doNotStack means NOT stackable
    }
  }, [bolData]);
