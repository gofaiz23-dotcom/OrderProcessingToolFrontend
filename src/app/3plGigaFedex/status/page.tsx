...
export default function StatusPage() {
  ...
  const [activeTab, setActiveTab] = useState<'excel' | 'scrapbol'>('excel');
  const hasNavigatedRef = useRef({ excel: false, scrapbol: false }); // Track navigation to prevent duplicates
  ...
  const fetchStatuses = async () => {
    try {
      setError(null);
      // Fetch both statuses
      const [excelResponse, scrapBolResponse] = await Promise.all([
        getAllUploadStatus(),
