...
export const InboxEmailsView = () => {
  ...
  // Get page number from URL pathname - supports /email/inbox/page2 format
  const currentPage = useMemo(() => {
    // Check if pathname matches /email/inbox/page{NUMBER}
    // This handles both /email/inbox/page2 and /email/inbox/page/2 formats
    const pageMatch = pathname.match(/\/email\/inbox\/page(\d+)$/);
    if (pageMatch) {
      const pageNum = parseInt(pageMatch[1], 10);
      return isNaN(pageNum) || pageNum < 1 ? 1 : pageNum;
    }
    // Also check for /email/inbox/page/{number} format
    const pageMatch2 = pathname.match(/\/email\/inbox\/page\/(\d+)$/);
    if (pageMatch2) {
      const pageNum = parseInt(pageMatch2[1], 10);
      return isNaN(pageNum) || pageNum < 1 ? 1 : pageNum;
    }
    // Default to page 1 for /email/inbox
    return 1;
  }, [pathname]);
  ...
  const [allEmails, setAllEmails] = useState<InboxEmail[]>([]);
  const [enrichedEmails, setEnrichedEmails] = useState<Map<string, InboxEmail>>(new Map());
  ...
  const selectedEmail = useMemo(() => {
    ...
    const enrichedEmail = enrichInboxEmail(lightweight);
  ...
  const fetchInboxEmailsData = useCallback(async (isInitialLoad: boolean = true) => {
    ...
    try {
      ...
      const data = await loadInboxEmails(fetchFilters, true, true, requiredCount);
  ...
  useEffect(() => {
    // Fetch emails - cache is checked FIRST inside loadInboxEmails
    // So if emails are cached, no API call will be made
    fetchInboxEmailsData(true);
  ...
  useEffect(() => {
    ...
    if (allEmails.length < requiredForNextPage && !loading && !loadingMore) {
      ...
      loadInboxEmails(fetchFilters, true, true, requiredForNextPage)
        .then((data) => {
  ...
  const handleEmailSelect = useCallback((email: InboxEmail) => {
    ...
    const enriched = enrichInboxEmail(email);
    if (enriched !== email) {
  ...
  const handleNavigatePrevious = useCallback(() => {
    if (currentPage > 1) {
      ...
      if (newPage === 1) {
        ...
        router.push(`/email/inbox/page${newPage}`);
      }
    }
  }, [currentPage, router]);
  ...
  const handleNavigateNext = useCallback(async () => {
    ...
    router.push(`/email/inbox/page${nextPage}`);
